...
import { getInitials, formatRelative, formatDate } from "@/lib/format";
import { ROUTES } from "@/lib/constants";
import type { OffboardingListItem } from "@/types/domain";
import { OffboardingStatusBadge } from "./offboarding-status-badge";

/** The Offboarding management table (admin + mentor). */
export function ManagementList({ rows }: { rows: OffboardingListItem[] }) {
  return (
        ...
        <table className="w-full min-w-[640px] text-sm">
          <thead>
            <tr className="border-border text-muted-foreground border-b text-left text-xs">
              ...
              <th className="px-4 py-2.5 font-medium">Last day</th>
          ...
          <tbody className="divide-border divide-y">
            {rows.map((row) => {
              const href = ROUTES.offboardingEditor(row.internshipId);
              return (
                <tr
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    {row.lastDay ? formatDate(row.lastDay) : "—"}
                  </td>
                  <td className="px-4 py-3">
                    <OffboardingStatusBadge status={row.status} />
                  </td>
